...
// requireAuth/logResult/handler factories take the env-derived secrets as
// arguments rather than reading process.env directly, so the same functions
// serve both vite.config.js (dev, via loadEnv) and server/index.js (prod, via
// process.env) without either side needing to know how the other sources them.
export function createApiHandlers({ sharedKey, anthropicApiKey }) {
  function requireAuth(req, res) {
    if (req.headers['x-stride-key'] !== sharedKey) {
  ...
  return { handleLocker, handlePhotos, handleSuggest, handleShade }
}
